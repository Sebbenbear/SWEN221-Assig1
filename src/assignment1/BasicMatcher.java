...
public class BasicMatcher implements Matcher {
	...
	public boolean match(String text, String regex) {
		...
		do {										//do this at least once
			...
			if (text.length() == 0 && regex.length() == 0) {				//if there's nothing in the text, return true?
				return true;
			}
			if(text.length() == 0){
				return false;
	...
	protected boolean matchHere(String text, String regex) {
		...
		if (regex.charAt(0) == '.'){
			...
			return matchHere(text.substring(1), regex.substring(1));  //changing to 0 makes lots more tests fail, fix
		}
	...
	protected boolean matchChar(String text, String regex) {
		...
		return regex.charAt(0) == '.' || (regex.charAt(0) == text.charAt(0) &&
					regex.charAt(0) != '*');
	}
